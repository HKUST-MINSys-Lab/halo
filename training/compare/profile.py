...
Run with OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 python -m training.compare.profile --out /tmp/profile.json.
...
from training.compare.train import (
...
from training.tokenizer.pretrain_episodic import _autocast, _random_encoder
...
def main():
    ...
    try:
        for workers, loader in loaders.items():
            torch.manual_seed(7)
            encoder, _ = _random_encoder(device, "fixed", neutral_acquisition_text=True)
            encoder.train()
