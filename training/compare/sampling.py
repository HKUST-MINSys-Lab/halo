...
DEPLOYMENT-MATCHED STAGED TRAINING
----------------------------------
The current staged readouts draw exact k-shot support for every candidate, then reuse that support
set across several distinct query executions. Multiple windows from one physical execution may be
...
The older sampler remains for historical readouts. In that path K may shrink and the shrink is
reported, but incompatible rows are likewise never used as padding.
"""
