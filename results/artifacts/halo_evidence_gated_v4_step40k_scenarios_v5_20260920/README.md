# HALO classifier v4, step 40k — deployment scenarios
...
Seven scenarios at 8 seconds and `k = 0, 1, 4, 8, 32`; manifests identical to the released-baseline
scenario artifact on all 737 shared cells. The headline change is partial coverage, where the
per-candidate blend weight rebalanced the trade-off it was designed to rebalance: truth-enrolled
accuracy at k=8 rises from 66.7 to 80.8 while truth-unenrolled falls from 53.9 to 44.5, lifting
overall accuracy (60.1 -> 62.3) and lowering the harmonic mean (59.7 -> 57.4).
