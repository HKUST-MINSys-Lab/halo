...
def test_neutral_acquisition_text_moves_modality_to_structured_metadata(index):
    key = next(key for key in index.train if all(index.refs[key.stream_i].mask))
    item = PretrainDataset(
        index, [key], augment=False, neutral_acquisition_text=True,
    )[0]

    assert item["sensor_texts"] == [
        "a device at an unspecified placement",
        "a device at an unspecified placement",
    ]
    assert item["sensor_modality"].tolist() == [0, 1]
