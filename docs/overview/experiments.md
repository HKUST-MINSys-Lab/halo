...
| T1-E | inductive-floor check: T1-C's encoder must not fall below v4's tier-2 1-NN k-curve | `halo-sealed-eval --models halo --halo-checkpoint <T1-C> --readouts 1nn ...` | ~20 min | **done** 03:56 HKT: **fails by ~2** (1-NN k=1/8/32: −1.9/−2.2/−2.5 vs v4) → `results/artifacts/rung1_halo_trained_20260926/INDUCTIVE_FLOOR.md` |
| T1-F | corpus-matched baseline arms (tier "2" of rung 1): HARNet, LiMU-BERT, UniMTS trunks trained on our corpus with differentiable neighbours, then scored like T1-A | `halo-train --encoder-arch {harnet,limubert,unimts} --classifier neighbors --steps 40000`; then `halo-rung1 --encoder-label matched:<arm>` | HARNet 32 min, LiMU-BERT ~75 min, UniMTS 13.6 h (cloud) | HARNet **done** 04:45 HKT: 33.7 / 39.3 / 40.8 (anchor / N=0 / N=all) → `results/artifacts/rung1_matched_harnet_20260926/` (training peaks ~22 GB GPU — run it alone); LiMU-BERT, UniMTS not started |
