...
def load_grid(dataset: str, stream: str):
    ...
    loader cannot open them). Returns windows, labels, subjects, channels, rate, and lengths."""
    ...
    for ds in TRAIN_DATASETS:
        for stream in eval_data.list_streams(ds, alignment="native"):
            ...
            if max_per_stream is not None and len(windows) > max_per_stream:
                ...
                windows, labels, subjects, lengths = (
                    windows[sel], labels[sel], subjects[sel], lengths[sel]
                )
